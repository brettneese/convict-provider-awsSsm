...
module.exports.parseParameters = function(parameters) {
  let r = {};
  let self = this;

  if (parameters.length > 0) {
    parameters.forEach(function(parameter) {
      let k = self.key(parameter.Name);
      r[k] = parameter.Value;
    });
  }
 ...
 * @example
 *  console.log(this.key('/i/am/a/key')) // "/i/am/a"
 */
 ...
 *
 */
module.exports.getValue = function(path) {
  let key = this.key(path);
  let basePath = this.basePath(path);
  ...
  if (!RESULTS[basePath]) {
    ...
    let r = this.parseParameters(parameters, basePath);
