...
// @todo write tests
// @todo write docs
objectify = function(path, value, returnObj = {}) {
  let firstPath = path[0];
  path.shift();
  returnObj[firstPath] = {};

  if (path.length > 0) {
    this.objectify(path, value, returnObj[firstPath]);
  } else {
    return (returnObj[firstPath] = value);
  }
...
// @todo write docs
// @todo write tests
module.exports.getValues = function(schema, property, getterFunction) {
  let merged = {};
  ...
  Object.keys(flattenedSchema).forEach(function(key) {
    ...
    if (k.pop() == property) {
      let value = getterFunction(flattenedSchema[key]);
      let o = this.objectify(k, value);
